...
#include "RefineImageInteractor.h"
namespace
{
  ...
  static RefineImageInteractor::Pointer CreateRefineImageInteractor()
  {
    auto exampleModule = us::ModuleRegistry::GetModule("MitkRefineModule");

    if (nullptr != exampleModule)
    {
      auto interactor = RefineImageInteractor::New();
...
const std::string QmitkRefineView::VIEW_ID = "org.mitk.views.exampleview";

void QmitkRefineView::CreateQtPartControl(QWidget* parent)
{
...
void QmitkRefineView::SetFocus()
{
  m_Controls.processImageButton->setFocus();
}

void QmitkRefineView::OnSelectionChanged(berry::IWorkbenchPart::Pointer, const QList<mitk::DataNode::Pointer>& dataNodes)
{
...
void QmitkRefineView::ProcessSelectedImage()
{
  ...
  if (data != nullptr)
  {
    ...
    if (image.IsNotNull())
    {
      ...
      // is pressed.
      auto interactor = CreateRefineImageInteractor();
